...
import { Sun, Moon, Monitor, Trash2, Download } from "lucide-react";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch("/api/stats")
      .then((res) => res.json())
      .then((data) => {
        setStats({ total: data.totalArticles, failed: data.failedTasks });
      });
  }, []);
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-col gap-4">
              <p className="text-sm text-[hsl(var(--muted-foreground))]">选择界面主题</p>
            ...
            {stats.failed > 0 && (
              <div className="flex items-center justify-between p-4 rounded-lg bg-red-50 dark:bg-red-950">
                <div>
                  ...
                  <p className="text-sm text-red-500">有 {stats.failed} 个失败任务可以清理</p>
                </div>
                {clearConfirm ? (
                  <div className="flex gap-2">
                    ...
                    <Button variant="destructive" size="sm" onClick={clearFailedTasks}>
                      确认清理
                    </Button>
                  </div>
                ) : (
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2 text-sm text-[hsl(var(--muted-foreground))]">
              <p>知乎内容库 v3.0</p>
              <p>基于 Remix + Cloudflare D1 + Drizzle 构建</p>
              <p>使用 HTTP 请求进行内容采集</p>
            </div>
          </CardContent>
        </Card>
      </div>
