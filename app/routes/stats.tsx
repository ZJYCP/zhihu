...
import { FileText, TrendingUp, Calendar, AlertCircle, Clock } from "lucide-react";
        ...
        <div key={item.date} className="flex-1 flex flex-col items-center gap-2">
          <div className="w-full flex flex-col items-center">
            ...
            <div
              ...
              style={{ height: `${Math.max((item.count / maxCount) * 80, 4)}px` }}
            />
          </div>
  ...
  return (
    <main className="container mx-auto max-w-6xl p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
        <StatCard title="文章总数" value={stats.totalArticles} icon={FileText} />
        <StatCard
          title="本周新增"
        ...
        />
        <StatCard title="本月新增" value={stats.monthArticles} icon={Calendar} description="最近 30 天" />
        <StatCard
          title="失败任务"
            ...
            {stats.recentArticles.length > 0 ? (
              <div className="space-y-3">
                {stats.recentArticles.map((article) => (
                  ...
                  >
                    <p className="font-medium line-clamp-1">{article.title || "无标题"}</p>
                    <p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">
              ...
              <p className="text-[hsl(var(--muted-foreground))] text-center py-8">暂无文章</p>
            )}
          </CardContent>
        </Card>
      </div>
